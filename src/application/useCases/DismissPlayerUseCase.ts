...
export class DismissPlayerUseCase {
  constructor(
  ...
  async execute(idPlayer: string, idTeam: string): Promise<boolean> {
    const findPlayer = await this.teamRepo.findPlayerById(idPlayer)
    ...
    team.sellPlayer(player.id)
    await this.marketRepo.addPlayerToMarket(player.toJSON())
    await this.teamRepo.removePlayerTeam(player.id)
